...
`zero lint` also runs an idiom pass over `<root>/src/**.{ts,js,tsx,jsx}`. Rules are hard-coded; no config.

| Rule | Trigger | Message gist |
| --- | --- | --- |
| R01 | `${signal.val}` inside ``html`` | reading `.val` breaks reactivity — pass the signal itself |
| R02 | `signal.val = ...` | signals are immutable from the outside — use `.set()` / `.update()` |
| R03 | module-level `signal/computed/effect` (outside `src/stores/**` and `src/app.{ts,js,tsx,jsx}`) | leaks; move into a function, the app entry, or a store factory |
| T01 | `addEventListener` / `removeEventListener` in `src/{components,routes}/**` | use `@event=` |
| T02 | unknown `@event.modifier` in ``html`` | typo — see allowed set in `runtime/template.js` |
| T03 | `each(items, render)` (no `keyFn`) | pass a key fn for stable identity |
| T04 | `document.querySelector` / `el.appendChild` etc. in `src/{components,routes}/**` | use `ref()` for element handles |
...
| I01 | bare specifier outside the allowlist (`zero`, `zero/components`, `zero/http`, `zero/test`) | no node_modules in zero |
...
Tests (`*.test.{ts,js,tsx,jsx}` / `*.spec.{ts,js,tsx,jsx}`) are exempt from the T-rules and R03; R02, C01, C02, I01, I02, S01 still apply.
...
- [x] R03 — module-level `signal/computed/effect` (exempt: `src/stores/**`, `src/app.{ts,js,tsx,jsx}`, tests)
