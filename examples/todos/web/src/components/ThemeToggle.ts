 ...
 * primitive. Wraps a local `Signal<boolean>` (true = dark) and mirrors
 * changes into the app-level theme store. The startup "follow system"
 * state is encoded as `theme.val === null`; the toggle seeds its
 * initial position from `prefers-color-scheme`.
 *
 * @returns Template.
 */
